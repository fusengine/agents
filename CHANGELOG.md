...
- fix(shadcn-expert): bump to 1.0.18 to ship the opus agent model — PR #57 changed `shadcn-ui-expert.md`'s model from `sonnet` to `opus` but `shadcn-expert`'s `plugin.json`/marketplace version was left unbumped in 1.39.30, so `/plugin update` would not have shipped it. (fuse-shadcn-ui 1.0.18)

## [1.39.30] - 24-09-2026

- feat(agents): switch all 38 plugin agents to `model: opus` (36 were `sonnet`; `challenger` and `design-expert` were already `opus`) — `opus` alias resolves to the latest Opus release. Add an Automatic Mode section to the `commit` agent: a relayed "commit" from the owner is treated as the go with no further questions on branch, scope, message, or version — one code commit plus one bump commit, never split or excluded, stop only on a real blocker; PATCH bump by default unless the mandate says otherwise. Add a new `fuse-ai-pilot:skill-creator` agent (executes the skill-creator skill, no `Agent` tool), wired into `plugins/ai-pilot/README.md`, `docs/plugins/ai-pilot.md`, and `docs/workflow/agents.md`. Add a `CLAUDE_CODE_SUBAGENT_MODEL` installer step (`scripts/src/services/subagent-model-gate.ts`, `p.select` opus/sonnet/haiku/unset, opus default and recommended, persisted via `writeRoutedVar`, keep/replace when already set) wired into `setup-runner.ts` after `promptHarnessGates`, with test coverage. `shadcn-expert`'s `plugin.json`/marketplace version was left unbumped: the repo's shadcn-skill research gate could not be satisfied in this session (no MCP research tool available to this agent) — flagged for the owner, versions stay consistent (1.0.17 in both files, no mismatch). (fuse-ai-pilot 1.2.42, fuse-astro 1.0.13, fuse-cartographer 1.0.13, fuse-changelog 1.0.16, fuse-commit-pro 1.2.26, fuse-lessons 1.0.8, fuse-go 1.0.7, fuse-laravel 1.2.8, fuse-nextjs 1.1.24, fuse-php 1.0.7, fuse-prompt-engineer 1.1.12, fuse-react 1.0.21, fuse-rust 1.0.7, fuse-security 1.0.19, fuse-seo 1.0.12, fuse-solid 1.0.18, fuse-swift-apple-expert 1.1.20, fuse-tailwindcss 1.1.11, fuse-tanstack-start 1.0.6, fuse-typescript 1.0.7)
...
- fix: align agents, skills and rules with current Claude Code API — expand agent `tools:` with `SendMessage`, `Write`, `Bash`, `mcp__sequential-thinking__sequentialthinking` and rename `Task` to `Agent` (37 agents); replace `TeamCreate`-based skill instructions (removed in Claude Code v2.1.178) with `Agent` calls carrying a `name` (116 skill files, incl. `agent-creator`); rewrite claude-rules Scope Ladder for exclusive-file-ownership splits and `SendMessage` coordination; `perf-env.ts` re-enables `CLAUDE_CODE_ENABLE_TODO_TOOLS` by default via a one-shot `applyDefaultPerfEnv()`; bump `@types/bun` `^1.3.14` -> `^1.4.0`. (fuse-ai-pilot 1.2.40, fuse-astro 1.0.12, fuse-cartographer 1.0.11, fuse-changelog 1.0.14, fuse-rules 1.0.21, fuse-commit-pro 1.2.25, fuse-design 2.2.8, fuse-lessons 1.0.6, fuse-go 1.0.5, fuse-laravel 1.2.6, fuse-nextjs 1.1.22, fuse-php 1.0.5, fuse-prompt-engineer 1.1.11, fuse-react 1.0.19, fuse-rust 1.0.5, fuse-security 1.0.17, fuse-seo 1.0.10, fuse-shadcn-ui 1.0.16, fuse-solid 1.0.17, fuse-swift-apple-expert 1.1.18, fuse-tailwindcss 1.1.9, fuse-tanstack-start 1.0.4, fuse-typescript 1.0.5)
...
- feat(design-expert): grow the design reference corpus from 10 to 15 — `mosa-recode` (Sylva AI, reproducing `mosa-ai.nextjsshop-preview.workers.dev`), `stash-recode` (Trove.link), `parley-recode` (Salut), `dispatch-recode` (Sodium), `stripe-recode` (Solvo, reproducing `stripe.com/fr`). Each reproduces its source page identically — only brand name, colors, and images change. `parley` is the first light-locked page in a corpus that was until now entirely dark; `stripe` is the densest page and the only one where saturated color is structural. The five add 22 local `.webp` photographs — the 10 historical references had no local assets and loaded everything remotely, which `refs-design/README.md` claimed as a corpus-wide trait; that claim, and the "single visual register" claim, are both corrected. `pre-flight-checklist.md` gains an 18th gate item (was 17): the report must now name the subject's verb and the reference retained with the procedure borrowed from it — a criterion that already existed in `refs-design/README.md` but sat below the index, unread before the choice was made, and unverified anywhere. Check 3's theme-inversion count moves from 0-of-10 to 1-of-15: `stripe-recode` reproduces a dark mid-scroll block present in its source, the exception is declared a failure in its own report rather than argued away, and the check stays blocking on original work. `MOTION_INTENSITY`'s dial ceiling rises from 6 to 7 — `dispatch-recode` runs a parallax no prior corpus page used. Two open items, not fixed here: `_artistic/elysian` remains a 16th reference outside the index, and the offline-autonomy table at `harness-recode/tokens-harness.md:814-826` is now stale but untouched, since it lives inside a delivered reference folder. (fuse-design 2.2.6)

## [1.39.23] - 30-07-2026

- fix(env-shell): stop leaking per-harness `FUSE_*` keys into every shell's global environment. `~/.claude/.env` declares `FUSE_HARNESS_REFS`, `FUSE_SOLID_MAX_LINES`, `FUSE_ENFORCE_TTL_SEC` — these are per-harness (`FUSE_HARNESS_REFS` points at Claude's own rules tree under `~/.claude/plugins/marketplaces/...`), and `@fusengine/harness` (`src/config/dotenv.ts:52`) deliberately never overwrites a key already present in `process.env`. Consequence, silent: a Codex or Kimi run launched from a shell carrying these exports read Claude's SOLID rules — including its line-count ceiling and TTL — instead of its own. All four loaders (`claude-env.bash`, `.zsh`, `.fish`, `.ps1`) now parse `~/.claude/.env` line by line and skip any `FUSE_`-prefixed key instead of `source`-ing (or fish's `grep '^export'`-based re-export of) the whole file; API keys (`CONTEXT7_API_KEY`, `EXA_API_KEY`, `MAGIC_API_KEY`, `GEMINI_DESIGN_API_KEY`, …) still export normally since MCP servers depend on them. Fish additionally routes non-interactive bash (used by Claude Code hooks) through a new `bash-env-loader.sh` shim instead of pointing `BASH_ENV` at the raw `.env` — a `source ~/.claude/.env` there would have re-leaked exactly what the fish config just filtered out; the shim re-reads the `.env` on every invocation, so it never needs resyncing with it. `install-env.sh`'s POSIX installer switched its heredoc from double- to single-quoted (`<< 'POSIX_LOADER'`) — the previous double-quoted form expanded `$HOME` and `${key%%=*}`-style parameter substitutions at *install* time, baking the installing user's values into every future invocation instead of leaving them to evaluate at *shell-start* time. `~/.claude/.env` itself and the `upsertEnvVar` writer are unchanged — the harness reads that file directly and must keep its `FUSE_*` entries; only the shell **exports** were the leak. Known gaps carried forward, not fixed here: PowerShell parsers (`claude-env.ps1`, `install-env.ps1`'s PS branch) are verified syntactically only — no `pwsh` available to run them; and machines already configured receive no update, because the idempotence guards (`scripts/src/services/shell-detection.ts:51,56,61`, `install-env.sh:67,120,146`, `install-env.ps1:35`) test for the substring `"claude"` rather than file content or a version marker, so an rc file with the old `source ~/.claude/.env` line still matches as "already installed" and is never rewritten.

## [1.39.22] - 29-07-2026

- fix(design-expert): balance the design gate with positive doctrine instead of only bans — three test pages (prestige real estate, driving school, architecture firm) passed every existing check and were still judged dead or undeliverable, because the whole apparatus was written entirely in the negative (dozens of prohibitions, almost no positive model): a page that breaks no rule scored 100% and stayed lifeless. `pre-flight-checklist.md` gains 6 new blocking items (12-17, gate now 17 total) — dominant-block hierarchy measured on the render, unconditional vertical rhythm (the existing detector only fired when the macro-structure was also canonical), a centred container with a shared nav/content gutter, an image floor (zero `<img>` is an unconditional fail), a motion floor (presence, not only coherence), and a small-viewport legibility floor (14px effective rendered size at 360px, closing the gap left by `typography.md`'s Body-Size Floor exempting caption/label tokens). `photos-images.md` adds a source hierarchy (Unsplash default, direct URL, no key; Pexels second), positive photo-selection criteria, and the DOCUMENTS/ILLUSTRATES rule (stock banks are off-limits when a visual documents the client's own work/team/premises). `design-method/SKILL.md`'s screen-holding floor stops treating drawing as a neutral substitute for photography when the brief supplies photography, and states that drawing and photography reinforce each other. (fuse-design 2.2.5)
...
- feat(design-expert): add an 11-page design reference corpus (`skills/design-web/references/refs-design/` — cursor, elysian, endlesstools, fora, harness, linear, mainframe, reve, supercommon, umbrel, xai — each with index.html/styles.css/motion.js/tokens/design-system.md) that becomes the taste/procedure source design-expert routes designers through, plus a `body-sequence-bank.md` (11 body sequences) and `scripts/layout-check/` — a deterministic Playwright-based layout QA tool (text overflow, element overlap, CTA line-wrap, contrast, document overflow) mechanizing checks the design-review skill previously only described in prose. (fuse-design 2.2.3)
- feat(fuse-lessons): add the `lessons-compactor` agent — reads `MEMORY/LESSON.md`, classifies every entry as keep/duplicate/merge-group/superseded, and proposes a compacted file without ever writing it directly; `/lessons-compact` now delegates to it instead of classifying inline. (fuse-lessons 1.0.5)
