...
import { scanPlugins, extractHooks } from "../services/plugin-scanner";
...
describe("plugin-scanner", () => {
  ...
  describe("extractHooks", () => {
    ...
    test("replaces CLAUDE_PLUGIN_ROOT in commands", () => {
      const plugins = [
        {
          name: "test-plugin",
          path: "/path/to/plugin",
              ...
              PreToolUse: [
                {
                  hooks: [
                    {
                      type: "command",
                      command: "bash ${CLAUDE_PLUGIN_ROOT}/scripts/test.sh",
                    },
                  ],
                },
              ],
