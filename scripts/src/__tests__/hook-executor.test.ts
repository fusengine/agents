...
import { executeHooks, executeHook } from "../services/hook-executor";
...
describe("hook-executor", () => {
  ...
  describe("executeHooks", () => {
    ...
    test("executes multiple hooks in parallel", async () => {
      const hooks: ExecutableHook[] = [
        { command: "sleep 0.05 && echo 'hook1'", isAsync: false, pluginName: "p1" },
        { command: "sleep 0.05 && echo 'hook2'", isAsync: false, pluginName: "p2" },
        { command: "sleep 0.05 && echo 'hook3'", isAsync: false, pluginName: "p3" },
      ];
    ...
    test("first blocked hook stops with blocked result", async () => {
      const hooks: ExecutableHook[] = [
        ...
        { command: "echo 'blocked' >&2 && exit 2", isAsync: false, pluginName: "plugin2" },
