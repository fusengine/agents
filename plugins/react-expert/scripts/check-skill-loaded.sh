...
# Only Write/Edit on .tsx/.ts/.jsx/.js files
[[ "$TOOL_NAME" != "Write" && "$TOOL_NAME" != "Edit" ]] && exit 0
[[ ! "$FILE_PATH" =~ \.(tsx|ts|jsx|js)$ ]] && exit 0
[[ "$FILE_PATH" =~ /(node_modules|dist|build)/ ]] && exit 0

# Smart detection: only block React code (not Next.js - handled by nextjs-expert)
CONTENT=$(echo "$INPUT" | jq -r '.tool_input.content // .tool_input.new_string // empty')
if echo "$CONTENT" | grep -qE "(use client|use server|NextRequest|NextResponse|from ['\"]next)"; then
  exit 0  # Let nextjs-expert handle it
fi
if ! echo "$CONTENT" | grep -qE "(useState|useEffect|useContext|useReducer|from ['\"]react)"; then
  exit 0  # Not React code
fi
...
skill_was_consulted "react" "$SESSION_ID" "$PROJECT_ROOT" && exit 0

PLUGINS_DIR="$HOME/.claude/plugins/marketplaces/fusengine-plugins/plugins"
MSG="BLOCKED: React skill not consulted. READ ONE: "
MSG+="1) $PLUGINS_DIR/react-expert/skills/solid-react/SKILL.md | "
MSG+="2) $PLUGINS_DIR/react-expert/skills/react-19/SKILL.md | "
MSG+="3) Use mcp__context7__query-docs (topic: react). After reading, retry."
deny_block "$MSG"
