...
export function buildDirectorySection(config: StatuslineConfig): MenuOption[] {
	return [
		...
		{ value: "directory.git", label: `    ${chk(config.directory.showGit)} └─ Show git info`, hint: "" },
		{ value: "directory.branch", label: `    ${chk(config.directory.showBranch)} └─ Show branch`, hint: "" },
		{ value: "directory.dirty", label: `    ${chk(config.directory.showDirtyIndicator)} └─ Dirty indicator (*)`, hint: "" },
		{ value: "directory.staged", label: `    ${chk(config.directory.showStagedCount)} └─ Staged files count`, hint: "" },
		{ value: "directory.unstaged", label: `    ${chk(config.directory.showUnstagedCount)} └─ Unstaged files count`, hint: "" },
	];
}
...
export function buildDailySection(config: StatuslineConfig): MenuOption[] {
	return [
		...
		{ value: "daily.budget", label: `    ${chk(config.dailySpend.showBudget)} └─ Show budget`, hint: "" },
	];
}
...
export function buildGlobalSection(config: StatuslineConfig): MenuOption[] {
	return [
		...
		{ value: "global.labels", label: `  ${chk(config.global.showLabels)} 🏷️  Show labels`, hint: "" },
		{ value: "global.separator", label: `  🔗 Separator: ${getSeparatorDisplay(config.global.separator)}`, hint: "" },
		{ value: "global.compact", label: `  ${chk(config.global.compactMode)} 📦 Compact mode`, hint: "" },
	];
}
