...
At the **start** of this phase, record it (and the resolved task subject) in `.claude/apex/task.json` — the harness reads `tasks[current_task].phase` to brief sub-agents, so a stale/missing value shows them the wrong step:
