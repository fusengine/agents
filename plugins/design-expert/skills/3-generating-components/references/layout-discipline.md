...
description: Hard layout constraints for Gemini generation — hero hard numbers, eyebrow restraint, zigzag cap, bento cell count, section-repetition ban, CTA and density limits
when-to-use: Before every create_frontend/modify_frontend call — inject these into the XML blocks and verify them in the returned output
keywords: layout, hero, eyebrow, bento, zigzag, forbidden, density, cta, pre-flight
priority: critical
related: gemini-design-workflow.md, ui-visual-design.md, premium-patterns/PATTERNS.md
---
...
Failing any rule below is shipping broken work. Inject each into the Gemini XML blocks (`<layout>`, `<spacing>`, `<forbidden>`) and re-check them mechanically in the returned markup before accepting output.
...
- Verify button-text contrast against its background (WCAG AA: 4.5:1 body, 3:1 for 18px+). (Source repo §4.5.)
