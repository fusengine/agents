...
STOP. Output summary. **No tag here** — it is created POST-MERGE by the `commit` command's Step 8. Load `references/tag-timing.md` for the full squash-merge rationale and the standalone-use exception.
...
This MUST be a separate commit from the code changes. Never combine. **No tag here** — same squash-merge rationale as Step S2. Load `references/tag-timing.md` for details.
